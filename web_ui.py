...
- Supplies template variables expected by the provided index.html:
    • graph_div: HTML for the graph generated by Plotly
...
import mqtt_handler  # Used here only for consistency; its data_mode is not displayed in this template

# Import Plotly offline and graph objects for generating the graph.
...
logging.info("Web UI starting... (v1.6)")
...
def get_historical_sensor_graph():
    ...
    try:
        ...
        cursor = conn.cursor()
        # Query the most recent 50 rows; adjust LIMIT as desired.
        ...
        # Generate HTML div without including Plotly.js (assumes you load Plotly via CDN or separately)
        graph_div = plot(fig, output_type='div', include_plotlyjs=False)
        return graph_div
    except Exception as e:
        logging.error("Failed to generate graph: " + str(e))
        return "<div>Error generating graph.</div>"
...
def index():
    ...
    # Generate the historical data graph using Plotly
    graph_div = get_historical_sensor_graph()
    # Default settings values; adjust these as needed
    min_temp = 18.0
    max_temp = 25.0
    view_hours = 24
